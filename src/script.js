...
gltfLoader.load("./testTree.glb", (gltf) => {
  particles = {};

  const model = gltf.scene.children[0]; // Supposons qu'il y ait un seul enfant (le mesh)
  const positions = model.geometry.attributes.position.array;

  const maxCount = positions.length / 3;

  particles.positions = new Float32Array(maxCount * 3);
  ...
  for (let i = 0; i < maxCount; i++) {
    sizesArray[i] = Math.random(); // ou toute autre logique pour définir la taille des points
  }
  particles.geometry.setAttribute(
    "aSize",
  ...
  particles.material = new THREE.ShaderMaterial({
    ...
    uniforms: {
      ...
      uProgress: new THREE.Uniform(-10),
